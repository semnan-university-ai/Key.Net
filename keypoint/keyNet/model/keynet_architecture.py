...
class keynet(object):
    ...
    def compute_handcrafted_features(self, image, network, idx, name_scope):
        ...
        dx = tf.keras.layers.Conv2D(image, self.kernel_filter_dx, strides=[1, 1, 1, 1], padding='SAME')
        dxx = tf.keras.layers.Conv2D(dx, self.kernel_filter_dx, strides=[1, 1, 1, 1], padding='SAME')
        dx2 = tf.multiply(dx, dx)

        # Sobel_conv_derivativeY
        dy = tf.keras.layers.Conv2D(image, self.kernel_filter_dy, strides=[1, 1, 1, 1], padding='SAME')
        dyy = tf.keras.layers.Conv2D(dy, self.kernel_filter_dy, strides=[1, 1, 1, 1], padding='SAME')
        dy2 = tf.multiply(dy, dy)

        dxy = tf.keras.layers.Conv2D(dx, self.kernel_filter_dy, strides=[1, 1, 1, 1], padding='SAME')
    ...
    def local_norm_image(self, x, k_size=65, eps=1e-10):
        ...
        x_mean = tf.nn.avg_pool2d(x_pad, ksize=[1, k_size, k_size, 1], strides=[1, 1, 1, 1], padding='VALID')
        x2_mean = tf.nn.avg_pool2d(tf.pow(x_pad, 2.0), ksize=[1, k_size, k_size, 1], strides=[1, 1, 1, 1],
                                 padding='VALID')
    ...
    def compute_features(self, input_data, dim, reuse, is_training):
        ...
        for idx_level in range(self.pyramid_levels):
            ...
            if idx_level == 0:
                ...
                input_data_smooth = tf.keras.layers.Conv2D(input_data, self.gaussian_avg, strides=[1, 1, 1, 1], padding='SAME')

            input_data_resized = tf.image.resize(input_data_smooth, size=tf.cast(
            ...
            features_t = tf.image.resize(features_t, size=tf.cast((dim_float[1], dim_float[2]), tf.int32),
    ...
    def conv_block(self, features, name, reuse, is_training, num_filters, size_kernel, batchnorm=True, activation_function=True):
        ...
        features = tf.keras.layers.Conv2D(inputs=features, filters=num_filters,
        ...
        if batchnorm:
            features = keras.layers.BatchNormalization(inputs=features, scale=True, training=is_training,
