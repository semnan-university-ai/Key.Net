...
import drive.MyDrive.keypoint.keyNet.datasets.dataset_utils as tools
...
class tf_dataset(object):
    ...
    def _compute_num_examples(self):
        ...
        self.num_examples = 0
        for _ in tf.data.TFRecordDataset(self.tfrecord_path):
            self.num_examples += 1

        self.num_val_examples = 0
        for _ in tf.data.TFRecordDataset(self.tfrecord_val_path):
