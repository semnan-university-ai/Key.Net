...
def ip_layer(scores, window_size, kernels):
    ...
    weights = tf.nn.max_pool2d(tf.stop_gradient(scores), [1, window_size, window_size, 1], strides=[1, window_size, window_size, 1], padding='VALID')
    ...
    exp_map_1 = tf.add(tf.pow(exponential_value, tf.math.divide(scores, max_pool_unpool+1e-6)), -1*(1.-1e-6))
...
def ip_softscores(scores, window_size, kernels):
    ...
    weights = tf.nn.max_pool2d(scores, [1, window_size, window_size, 1], strides=[1, window_size, window_size, 1], padding='VALID')
...
def grid_indexes_nms_conv(scores, kernels, window_size):
    ...
    ind_rand = tf.cast(tf.random.uniform(tf.shape(indexes_label), minval=0, maxval=window_size, dtype=tf.int32), tf.float32)
...
def msip_loss_function(src_im, src_score_maps, dst_score_maps, window_size, kernels, h_src_2_dst, h_dst_2_src,
    ...
    if coordinate_weighting:
        ...
        weights_src = keras.layers.flatten(weights_src)
        weights_dst = keras.layers.flatten(weights_dst)
